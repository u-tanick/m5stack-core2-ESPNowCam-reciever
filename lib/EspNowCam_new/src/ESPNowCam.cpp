...
xSemaphoreHandle xSendSemaphore;
...
bool ESPNowCam::sendData(uint8_t *data, uint32_t lenght) {
  ...
  while (frame_left > 0) {
    ...
    xSemaphoreTake(xSendSemaphore, 0); // Clear semaphore before sending
    sendMessage(encodeMsg(msg), this->targetAddress);
    if (xSemaphoreTake(xSendSemaphore, pdMS_TO_TICKS(100)) == pdTRUE) { // Wait for send to complete with a timeout of 100ms
      // 送信成功
    } else {
      // 送信失敗（タイムアウト）
      log_e("ESPNow send timeout or peer not found.");
      // ここでエラー処理を行うか、フレームの送信を中止するなどの対応を検討
      // 例: return false; で関数を終了
      chunk_pos = 0; // 現在のフレームの送信を中止し、次のフレームに進まないようにする
      return false;
    }
...
struct_receiver *curReceiver = nullptr;

/// filters map (id,macaddr)
...
bool checkReceiver(const uint8_t *macAddr) {
  ...
  if (pos != buffers.end()) return true;
  return false;
}
...
void saveReceiver(const uint8_t *macAddr, uint8_t *fb, RecvCb cb) {
  if (!checkReceiver(macAddr)) {
    uint32_t id = getReceiverId(macAddr);
    log_i("[%02d] New receiverId: %i with MAC:", buffers.size()+1, id);
    printMacAddress(macAddr);
    struct_receiver rcv;
    rcv.fb = fb;
    rcv.fbpos = 0;
...
bool ESPNowCam::init(uint8_t chunk_size) {
  ...
  if (esp_now_init() == ESP_OK) {
    ...
    xSendSemaphore = xSemaphoreCreateBinary(); // Initialize semaphore
